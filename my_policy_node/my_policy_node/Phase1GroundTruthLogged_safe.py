...
class Phase1GroundTruthLogged(Policy):
    ...
    # Optional joint-space prepose. This is the only motion-logic addition:
    # it moves the arm to an elbow-away posture before the existing Cartesian
    # approach/descent starts. Set this to False to recover the exact previous
    # behavior.
    USE_SAFE_JOINT_PREPOSE = True

    # First-guess elbow-away posture in radians.
    # Tune these visually if the custom board pose still puts the elbow in a bad place.
    SAFE_JOINT_POSITIONS = [
        -1.20,   # shoulder_pan_joint
        -1.70,   # shoulder_lift_joint
         1.85,   # elbow_joint
        -1.75,   # wrist_1_joint
        -1.57,   # wrist_2_joint
         0.00,   # wrist_3_joint
    ...
    SAFE_JOINT_STIFFNESS = [55.0, 55.0, 55.0, 35.0, 35.0, 35.0]
    SAFE_JOINT_DAMPING = [20.0, 20.0, 20.0, 12.0, 12.0, 12.0]

    SAFE_JOINT_STEPS = 140
    SAFE_JOINT_DT = 0.05
    SAFE_JOINT_TOL = 0.04
    SAFE_JOINT_STABLE_COUNT = 8
    ...
    # -------------------------------------------------------------------------
    # Joint-space prepose helpers.
    # -------------------------------------------------------------------------

    def _make_joint_motion_update(
        self,
        positions: list[float],
        stiffness: Optional[list[float]] = None,
        damping: Optional[list[float]] = None,
    ) -> JointMotionUpdate:
        """Create a joint-space command.

        JointMotionUpdate is the AIC joint-space equivalent of MotionUpdate:
        target_state contains joint positions/velocities/etc., and stiffness /
        damping are per-joint arrays.
        """

        if stiffness is None:
            stiffness = self.SAFE_JOINT_STIFFNESS
        if damping is None:
            damping = self.SAFE_JOINT_DAMPING

        msg = JointMotionUpdate()

        msg.target_state = JointTrajectoryPoint()
        msg.target_state.positions = [float(x) for x in positions]
        msg.target_state.velocities = [0.0 for _ in positions]
        msg.target_state.accelerations = [0.0 for _ in positions]
        msg.target_state.effort = [0.0 for _ in positions]

        msg.target_stiffness = [float(x) for x in stiffness]
        msg.target_damping = [float(x) for x in damping]
        msg.target_feedforward_torque = [0.0 for _ in positions]
    ...
    def _current_arm_joint_positions(
        self,
        get_observation: GetObservationCallback,
    ) -> Optional[list[float]]:
        """Return the current 6 arm joint positions from Observation.joint_states.

        Uses exact names first, then substring matching, then first-six fallback.
        The substring path makes this robust if the simulator prefixes joint names.
        """
        ...
        if all(name in name_to_pos for name in arm_names):
            return [name_to_pos[name] for name in arm_names]

        # Handle possible prefixes such as robot/shoulder_pan_joint.
        out = []
        for target_name in arm_names:
            match = None
            for name, pos in zip(names, positions):
        ...
        # Fallback: most AIC observations list arm joints first.
        if len(positions) >= 6:
            return [float(x) for x in positions[:6]]

        return None

    def _joint_error_norm(self, target: list[float], current: list[float]) -> float:
        """Shortest angular error norm for revolute joints."""

        err = np.asarray(target, dtype=float) - np.asarray(current, dtype=float)
        err = (err + math.pi) % (2.0 * math.pi) - math.pi
        return float(np.linalg.norm(err))

    def _move_to_safe_joint_prepose(
        self,
        get_observation: GetObservationCallback,
        move_robot: MoveRobotCallback,
    ) -> bool:
        """Move to an elbow-away joint posture before Cartesian insertion.

        This selects a better arm posture/IK branch for custom scene layouts.
        After this function returns, the existing Cartesian ground-truth approach
        and descent run unchanged.
        """

        target = [float(x) for x in self.SAFE_JOINT_POSITIONS]

        self.log_event(
            "PREPOSE",
            (
                "moving to safe joint prepose: "
                + ", ".join(f"{x:+.2f}" for x in target)
            ),
            C.CYAN,
        )
        ...
        for _ in range(self.SAFE_JOINT_STEPS):
            current = self._current_arm_joint_positions(get_observation)

            if current is not None and len(current) == len(target):
                err_norm = self._joint_error_norm(target, current)

                self.log_throttle(
                    key="safe_joint_prepose",
                    tag="PREPOSE",
                    msg=f"joint_err_norm={err_norm:.4f} stable={stable}/{self.SAFE_JOINT_STABLE_COUNT}",
                    color=C.CYAN,
                    period=0.5,
                )

                if err_norm < self.SAFE_JOINT_TOL:
                    stable += 1
                else:
                    stable = 0

                if stable >= self.SAFE_JOINT_STABLE_COUNT:
                    self.log_event(
                        "PREPOSE",
                        f"safe joint prepose reached, err_norm={err_norm:.4f}",
                        C.GREEN,
                    )
                    return True
            else:
                self.log_throttle(
                    key="safe_joint_no_obs",
                    tag="PREPOSE",
                    msg="waiting for joint state observation...",
                    color=C.YELLOW,
                    period=1.0,
                )
            ...
            try:
                ok = move_robot(
                    joint_motion_update=self._make_joint_motion_update(
                        positions=target,
                        stiffness=self.SAFE_JOINT_STIFFNESS,
                        damping=self.SAFE_JOINT_DAMPING,
                    )
                ...
                if ok is False:
                    self.log_throttle(
                        key="safe_joint_move_false",
                        tag="PREPOSE",
                        msg="move_robot returned False for joint prepose",
                        color=C.YELLOW,
                        period=1.0,
                    )
            except Exception as ex:
                self.log_event("PREPOSE", f"joint prepose command failed: {ex}", C.RED)
                return False

            self.sleep_for(self.SAFE_JOINT_DT)

        self.log_event(
            "PREPOSE",
            "safe joint prepose timed out; continuing with Cartesian approach anyway",
            C.YELLOW,
        )
        return False
    ...
    def insert_cable(
        ...
        self.log_event(
            "MODE",
            "Control logic is unchanged after prepose: inner port frame target + open-loop z descent; logging only.",
            C.MAGENTA,
        )

        # ---------------------------------------------------------------------
        # Phase 0: optional joint-space safe posture.
        #
        # This is the only motion-logic addition. It moves the elbow away before
        # Cartesian approach starts. Disable USE_SAFE_JOINT_PREPOSE to recover
        # the exact previous logged-baseline behavior.
        # ---------------------------------------------------------------------
        if self.USE_SAFE_JOINT_PREPOSE:
            send_feedback("Moving to safe joint prepose")
            self._move_to_safe_joint_prepose(
                get_observation=get_observation,
                move_robot=move_robot,
            )

            # Give the model/controller one tick before switching back to
            # Cartesian commands.
            self.sleep_for(0.2)
